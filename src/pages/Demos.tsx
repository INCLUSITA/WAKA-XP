    ...
    <div className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-6 transition-all hover:border-white/20 hover:bg-white/10 hover:shadow-2xl">
      ...
      <div className="mb-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {sourceLabel && (
            <p className="text-[10px] text-white/35 mt-0.5 truncate">Desde: {sourceLabel}</p>
          )}
        </div>
      </div>
...
export default function Demos() {
  ...
  /* ─ Duplicate builtin as sandbox ─ */
  const handleDuplicateBuiltin = (demo: DemoEntry) => {
    // For builtin demos we don't have JSX source, store a marker
    const sandbox = duplicateDemoAsSandbox(demo, `/* Sandbox copy of built-in demo: ${demo.id} */`);
    saveUploadedDemo(sandbox);
    refreshUploaded();
  };

  /* ─ Duplicate uploaded as sandbox ─ */
  const handleDuplicateUploaded = (demo: UploadedDemo) => {
  ...
  /* ─ Open demo ─ */
  const openDemo = (id: string, jsxSource?: string) => {
    if (jsxSource) sessionStorage.setItem(`demo-jsx-${id}`, jsxSource);
    navigate(`/demo/${id}`);
  };

  /* ─ Group uploaded demos by status ─ */
      ...
      <main className="mx-auto max-w-6xl px-6 py-8 space-y-10">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {BUILTIN_DEMOS.map((demo) => (
              <DemoCard
                ...
                demo={demo}
                status="stable"
                onOpen={() => navigate(`/demo/${demo.id}`)}
                onDuplicate={() => handleDuplicateBuiltin(demo)}
              />
            ))}
        ...
        <section className="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-8 py-8">
          ...
          <p className="text-xs text-white/30 max-w-lg">
            ...
            Sandbox demos will be the safe workspace for AI-suggested changes before promoting to stable.
          </p>
        </section>
        ...
        <button
          ...
          className="flex w-full cursor-pointer items-center justify-center rounded-2xl border-2 border-dashed border-white/15 p-6 text-center text-white/40 transition-all hover:border-white/30 hover:bg-white/5 hover:text-white/60"
        >
          <div>
